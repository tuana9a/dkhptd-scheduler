            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-username-password-tuana9a', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                    script {
                        ...
                        sed -i "s|image: tuana9a/dkhptd-scheduler:.*|image: tuana9a/dkhptd-scheduler:$BUILD_TAG_SECONDLY|" k8s/deployments.yaml
                        git add .
                        git commit -m "Update dkhptd-scheduler version to $BUILD_TAG_SECONDLY"
