            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'gitlab-username-password-tuana9a', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                    script {
                        ...
                        env.MR_BRANCH_NAME = "bump-dkhptd-scheduler-version-$BUILD_TAG_SECONDLY"
                        ...
                        sed -i "s|image: tuana9a/dkhptd-scheduler:.*|image: tuana9a/dkhptd-scheduler:$BUILD_TAG_SECONDLY|" deploy/dkhptd/k8s/deployments.yaml
                        git add .
                        git commit -m "Bump dkhptd-scheduler version to $BUILD_TAG_SECONDLY"
